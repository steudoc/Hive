...
posts = [{
        ...
        "commenti": [
             {"username": "luigi",
              "immagine_profilo": "/static/images/luigi.png",
              "testo": "hello, this is my first comment",
              "valutazione" : 3,
              "immagine_commento": "/static/images/no_image.png"},
              {"username": "steudoc",
...
user_list = [
    {
        "id": 0,
        "username": "anonimo", 
        "immagine_profilo": "/static/images/anonimo.jpg",
        "nome": "Anonimo 🤖",
        "bio": "Un misterioso esploratore del cyberspazio. 🌌💻",
        "hashtags": ["Anonimo", "Mistero"]
    }, {
        "id": 1,
        "username": "luigi",
        "immagine_profilo": "/static/images/luigi.png",
...
def post(post_id):
    ...
    return render_template('post.html', p_post=post_selected, p_users=user_list)
...
def new_comment(post_id):
    ...
    user = next((u for u in user_list if u['username'] == comment['username']), None)
    if user:
        comment['immagine_profilo'] = user['immagine_profilo']
    else:
        app.logger.error("Immagine profilo non trovata per l'utente!")
        comment['immagine_profilo'] = 'static/images/no_image.png'
    ...
    post['commenti'].append(comment)

    app.logger.info(f"Commento elaborato: {comment}")
...
def new_post():
    ...
    app.logger.info(f"Hashtags ricevuti: {hashtags}")
